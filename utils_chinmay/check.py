...
def get_interestPoints_from_parts_indices(visit_files):
    """Load indices from a JSON file."""
    ann_id_to_visit_id = {}
    for visit in visit_files:
        annotations_path = home +str(int(visit))+'/'+ visit + '_annotations.json'
        with open(annotations_path, 'r') as file:
            data = json.load(file)
            annotations = data['annotations']
            for ann in annotations:
                ann_id_to_visit_id[ann['annot_id']] = visit

    return ann_id_to_visit_id
...
ann_id_to_visit_id = get_interestPoints_from_parts_indices(visit_files)
