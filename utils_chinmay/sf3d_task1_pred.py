...
initialize(config_path="../conceptgraph/Mask3D/conf", job_name="test_app")  # Initialize Hydra
...
def sample_points(small_pcd, big_pcd):
    small_points = np.asarray(small_pcd.points)
    big_points = np.asarray(big_pcd.points)

    # Check if color information exists
    if small_pcd.has_colors() and big_pcd.has_colors():
        small_colors = np.asarray(small_pcd.colors)
        big_colors = np.asarray(big_pcd.colors)
    else:
        raise ValueError("One or both point clouds do not have color information.")

    # Build a KDTree for fast nearest-neighbor search
    tree = cKDTree(big_points)

    # Define number of extra points to sample
    num_samples = 20000  # Adjust based on requirement
    distance_threshold = 0.5  # Adjust based on required proximity

    # Find nearest neighbors in big PCD for each point in small PCD
    indices = tree.query_ball_point(small_points, distance_threshold)

    # Collect additional points and corresponding colors from big PCD
    new_sampled_points = []
    new_sampled_colors = []

    for idx_list in indices:
        if len(idx_list) > 0:
            new_sampled_points.extend(big_points[idx_list])
            new_sampled_colors.extend(big_colors[idx_list])

    # Convert to numpy and randomly sample required number of points
    new_sampled_points = np.array(new_sampled_points)
    new_sampled_colors = np.array(new_sampled_colors)

    if len(new_sampled_points) > num_samples:
        sampled_indices = np.random.choice(len(new_sampled_points), num_samples, replace=False)
        new_sampled_points = new_sampled_points[sampled_indices]
        new_sampled_colors = new_sampled_colors[sampled_indices]

    # Combine small PCD with new sampled points and colors
    combined_points = np.vstack((small_points, new_sampled_points))
    combined_colors = np.vstack((small_colors, new_sampled_colors))
...
def get_colors_for_id(number_of_colors):
    file_path = "/media/gokul/Elements1/scenefun3d/label_mapping.csv"
...
def generate_clusters(points, colors, eps=0.05, min_points=100):
    ...
    for point, color in zip(points, colors):
        ...
        #skip floor
        floor_color = '214-39-40'
        rgb_values = list(map(int, floor_color.split('-')))
        normalized_rgb = [val / 255.0 for val in rgb_values]
        clipped_rgb_floor = np.clip(normalized_rgb, 0, 1).tolist()    
        

        if(color[0] == color_to_compare[0] and color[1] ==  color_to_compare[1] and color[2] == color_to_compare[2]) or (color[0] == clipped_rgb_floor[0] and color[1] ==  clipped_rgb_floor[1] and color[2] == clipped_rgb_floor[2]):
            continue
...
def find_matching_clusters(id_to_label_and_points, clusters_by_color):
    ...
    for id, value in id_to_label_and_points.items():
        ...
        if matching_clusters:
            ...
            #sampling by distance starts
            if(len(merged_points)>21000):
                kdtree = cKDTree(value['points'])
                distances, _ = kdtree.query(merged_points)
                alpha = 1.0  # Adjust this parameter as needed
                probabilities = np.exp(-alpha * distances)

                # Step 5: Normalize probabilities
                probabilities /= probabilities.sum()

                # Step 6: Randomly sample points labeled 0
                num_samples = 21000  # Adjust the number of samples as needed
                sample_indices = np.random.choice(len(merged_points), size=num_samples, p=probabilities)
                sampled_points = merged_points[sample_indices]   
            else:
                sampled_points = merged_points
            #sampling by distance ends

            merged_pcd = o3d.geometry.PointCloud()
            merged_pcd.points = o3d.utility.Vector3dVector(sampled_points)

            # Create a color array for the merged_pcd (light grey)
            merged_colors = np.full((sampled_points.shape[0], 3), [0.8, 0.8, 0.8], dtype=np.float64)  # Light grey
            ...
            merged_points_set = set(map(tuple, sampled_points))
            ...
            if overlapping_points.size > 0:
                mask = np.isin(sampled_points, overlapping_points).all(axis=1)
...
def get_visitID_to_videoID_dict():
    # Load the Excel file
    file_path = '/media/gokul/Elements1/scenefun3d/union_of_scenes.xlsx'
...
scenefun_home = '/media/gokul/Elements1/scenefun3d/scenefun/'
labelmaker_home = '/media/gokul/Elements1/scenefun3d/arkitscenes/Training/'
...
save_path = '/media/gokul/Elements1/scenefun3d/part_object/'
...
for visit,video_list in dict__.items():
    if not int(visit) in  [423306, 426265, 423738, 422134, 437219, 422148, 423070, 464795, 435715, 423325, 435363, 434700, 438794, 466159, 467346, 421013, 468282, 467337, 422535, 438017, 421391, 466739, 466111, 468140, 420683, 464754, 467115, 434688, 423957, 437539, 421853, 467252, 467367, 437134, 438804, 467360, 466967, 468074, 468496, 421379 ]:
        continue
    ...
    for video in video_list:
        if video in [44796443, 42445168, 42897478, 47333876, 45662870, 47334515, 47334523, 47334522, 42897421, 42446164]:
        # if not video in [42445799]:
        ...
        label_to_common_cat = pd.read_excel("/media/gokul/Elements1/scenefun3d/wn199_to_common_cat_map.xlsx", engine='openpyxl')  # Load Excel file into a DataFrame
    ...
    for id, obj in id_part_obj.items():
        ...
        colors_final = []
        normals_final = []
        key_to_write = scenefun_eval[str(obj['meta_data']['part_label'])]
        save_id_path = save_path+'predicted_masks/'+str(int(visit))+'_'+str(count)+'.txt'
        with open(save_path+str(int(visit))+'.txt','a') as file_to_write:
            file_to_write.write('predicted_masks/'+str(int(visit))+'_'+str(count)+'.txt '+str(key_to_write)+' 1\n')
        for pcd in obj['pcds']:
            # Compute normals
            org_points = np.asarray(pcd2.points)
            ...
            for i in range(len(points)):
                ...
                # Create a tuple of the point coordinates and label for uniqueness
                point_tuple = (x, y, z)
                
                # Only write if the point is unique
                if point_tuple not in unique_points:
                    unique_points.add(point_tuple)  # Mark the point as written
                    points_final.append(points[i])
                    colors_final.append(color)
                    normals_final.append(normal)
                    # Format the data: x z y normal_value1 normal_value2 normal_value3 label
                    line = f"{x} {z} {y} {normal[0]} {normal[1]} {normal[2]} {label}\n"
                    #file.write(line)
        
        new_pcd = o3d.geometry.PointCloud()
        new_pcd.points = o3d.utility.Vector3dVector(np.asarray(points_final))
        new_pcd.colors = o3d.utility.Vector3dVector(np.asarray(colors_final))
        new_pcd.normals = o3d.utility.Vector3dVector(np.asarray(normals_final))
        #o3d.visualization.draw_geometries([new_pcd])
        if len(points_final) < 1000:
            new_pcd = sample_points(new_pcd, pcd2)
            points_final = np.asarray(new_pcd.points)
        print('ererrherherherh.......................................',len(new_pcd.points))
        masks_scores = mask3d_main(cfg_mask3d, np.asarray(new_pcd.points), np.asarray(new_pcd.colors),  np.asarray(new_pcd.normals), 'test')
        for key, value in masks_scores.items():
            if key == mask3d_keys[str(obj['meta_data']['part_label'])]:
                masks = np.array(value['masks'])

                # Aggregation using logical OR (binary mask)
                mask = np.any(masks, axis=0).astype(bool)

                final_segmentation = np.asarray(points_final)[mask]

                tree = cKDTree(org_points)

                # Find nearest neighbor in pcd2 for each point in pcd
                _, indices = tree.query(final_segmentation, k=1)  # k=1 for closest match

                print("Matching indices in pcd2 for annot :", key, indices)
                print('length of pcd and indices', len(points), len(indices))

                #insert line in visit.txt ( path(pred_masks/visit_count.txt) label conf)
                with open(save_id_path,'w') as file:
                    for index in list(indices):
                        file.write(str(index)+'\n')
                    count += 1
                #stroe indice in pred_masks/visit_count.txt
